...
- `docker compose exec -e TEST_DATABASE_URL=postgresql+asyncpg://jooba:jooba@db:5432/jooba -e ALLOW_TEST_DB_TRUNCATE=1 backend python -m pytest tests/test_enrollment_service.py tests/test_enrollment_schemas.py tests/test_candidate_repository.py tests/test_enrollments_api.py -v` - regression suite for CSV enroll/candidates stack.
- **Test DB truncation excludes `nylas_accounts`:** The user's real OAuth connection must survive test runs. `conftest.py` only truncates `sequences, candidates` (CASCADE handles dependents). Never add `nylas_accounts` back to `_TRUNCATE_TABLES`.
- **Classifier text priority:** `body_html` (full Nylas body) must be preferred over `body_text` (truncated Nylas snippet) for classification. Snippet truncation loses keywords needed for correct sentiment detection.
